...
static struct my_expr *new_expr()
{
 ...
 */
static unsigned find_parenthesis(struct my_token *tokens, unsigned index, unsigned length)
{
    unsigned level = 1;
...
static struct my_expr *parse_operations(
        struct my_token *tokens,
        unsigned *index,
        unsigned length
        );

static struct my_expr *parse_operand(
        struct my_token *tokens,
        unsigned *index,
        unsigned length
        )
{
    ...
    if (token->type == TOKEN_MINUS)
    {
        ...
        struct my_expr expr = { EXPR_NEGATION, { {
            parse_operand(tokens, index, length), NULL
        } } };
...
static int is_op_token(enum my_token_type token_type)
{
    return token_type < TOKEN_LEFT_PARENTHESIS;
}

static struct my_expr *build_op_mult(
        struct my_expr **expr_stack,
        ...
        )
{
    ...
    struct my_expr expr = { op_stack[op_stack_length - 1], { {
        build_op_mult(
                expr_stack, expr_stack_length - 1,
...
static struct my_expr *build_op_sum(
        struct my_expr **expr_stack,
        unsigned expr_stack_length,
        enum my_expr_type *op_stack,
        unsigned op_stack_length
        )
{
    ...
    for (unsigned i = op_stack_length - 1; i < op_stack_length; --i)
        ...
        {
            struct my_expr expr = { op_stack[i], { {
                build_op_sum(
                        expr_stack, i + 1,
                        op_stack, i
                        ),
                build_op_mult(
                        expr_stack + i + 1, expr_stack_length - (i + 1),
    ...
    return build_op_mult(
            expr_stack, expr_stack_length,
...
struct my_expr *parse_expr(char **strs, unsigned length)
{
    ...
    unsigned index = 0;
    return parse_operations(tokens, &index, length);
}
